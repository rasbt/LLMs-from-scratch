...
- The same OpenAI Python client also accepts `base_url` for OpenAI-compatible multi-model gateways (for example [DaoXE](https://daoxe.com?utm_source=github&utm_medium=organic&utm_campaign=llms-from-scratch&utm_content=ch07-eval) at `https://api.daoxe.com/v1`) when you are not calling `api.openai.com` directly: `OpenAI(api_key=..., base_url="https://api.daoxe.com/v1")`.
